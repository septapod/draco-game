...
- **Fix 4: Narrator verbosity** — Model was ignoring brevity instructions because conversation history was full of long responses that the model pattern-matched. Final approach: (1) hard 50-word narrative limit stated explicitly in system prompt at the end (strongest position), (2) "NEVER give multiple NPCs their own dialogue lines — at most ONE character speaks" rule, (3) explicit instruction to ignore length of previous responses, (4) max_tokens dropped from 500 to 200 as hard ceiling. Dramatic style (sound effects, bold, ALL CAPS) preserved — only length and multi-character dialogue constrained.
