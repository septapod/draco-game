...
- **Fix 4: Narrator verbosity** — Model was ignoring brevity instructions because conversation history was full of long responses that it pattern-matched. Hard max_tokens limits just caused truncation. Final approach: truncate assistant messages in conversation history to ~60 words before sending to the API (strips `<game_state>` blocks, keeps first 60 words of narrative). The full history stays intact for saves/compression, but the API only sees short responses — so the model naturally follows a short-response pattern. max_tokens set to 400 (safe for game_state JSON). System prompt still has 50-word limit + single-NPC-dialogue rules at the end as reinforcement.
