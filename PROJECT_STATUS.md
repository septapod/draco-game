...
- **Fix 4: Narrator verbosity** — Model was ignoring the "2-4 sentences MAX" brevity instruction, producing 15+ line dramatic responses. Three changes: (1) moved brevity rules to end of system prompt where they have strongest influence, (2) added good/bad examples — good shows dramatic style (sound effects, bold, ALL CAPS) in 4-6 lines; bad shows the actual failure mode (every dragon getting dialogue, 15+ lines of back-and-forth), (3) dropped max_tokens from 500 to 300 as hard ceiling. Dramatic style (sound effects, bold text, ALL CAPS hits) is explicitly preserved — only length is constrained.
