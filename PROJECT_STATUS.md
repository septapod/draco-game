...
- **Reverted: Narrator brevity experiments** — Attempted to shorten narrator responses via max_tokens reduction (500→300→200), prompt rewrites, and conversation history truncation. All approaches either truncated mid-sentence (breaking gameplay) or were ignored by the model. Reverted to original state: max_tokens=500, original brevity instructions in system prompt ("2-4 sentences MAX"), no history truncation. Narrator verbosity remains a known issue to address differently in the future (possibly post-processing or a separate summarization step).
